# ru-pills.com - Docker Infrastructure

Инфраструктура для быстрого развертывания и восстановления WordPress сайта ru-pills.com

## 📋 Содержание

- [Быстрый старт](#быстрый-старт)
- [Структура проекта](#структура-проекта)
- [Бэкапы](#бэкапы)
- [Восстановление](#восстановление)
- [Docker команды](#docker-команды)
- [GitHub](#github)

## 🚀 Быстрый старт

### Требования

- Docker и Docker Compose
- Git

### Установка

1. **Клонируйте репозиторий:**
```bash
git clone https://github.com/your-username/rupills-infra.git
cd rupills-infra
```

2. **Настройте переменные окружения:**
```bash
cp .env.example .env
nano .env  # Заполните пароли и настройки
```

3. **Запустите контейнеры:**
```bash
docker-compose up -d
```

4. **Установите WordPress (если первый запуск):**
```bash
docker-compose exec web wp core download --locale=ru_RU
docker-compose exec web wp core config --dbname=wordpress --dbuser=wordpress --dbpass=your_password
docker-compose exec web wp core install --url=ru-pills.com --title="ru-pills" --admin_user=admin --admin_password=secure_password
```

5. **Восстановите плагины из бэкапа:**
```bash
./scripts/restore.sh plugins
```

## 📁 Структура проекта

```
/var/www/
├── docker-compose.yml      # Docker Compose конфигурация
├── Dockerfile              # Образ OpenLiteSpeed
├── .env.example            # Пример переменных окружения
├── .gitignore              # Исключения для Git
├── scripts/
│   ├── backup-db.sh        # Бэкап базы данных
│   ├── backup-plugins.sh   # Бэкап плагинов
│   ├── backup-all.sh       # Полный бэкап
│   └── restore.sh          # Восстановление из бэкапов
├── html/                   # WordPress файлы
│   ├── wp-content/
│   │   ├── themes/
│   │   │   └── flatsome-child/  # Кастомная тема (в Git)
│   │   └── plugins/              # Плагины (НЕ в Git)
│   └── backup-download.php # Веб-интерфейс для скачивания бэкапов
└── backups/                # Директория для бэкапов
```

## 💾 Бэкапы

### Автоматические бэкапы

Настройте cron для автоматических бэкапов:

```bash
# Добавьте в crontab (crontab -e)
0 */6 * * * /var/www/scripts/backup-all.sh >> /var/www/backups/backup.log 2>&1
```

Это создаст бэкапы каждые 6 часов:
- База данных (db-latest.sql.gz)
- Плагины (plugins-latest.tar.gz)
- Uploads (uploads-latest.tar.gz)
- wp-config.php (wp-config-latest.enc - зашифрован)
- Тема flatsome-child (flatsome-child-latest.tar.gz)

### Ручной бэкап

```bash
# Полный бэкап
./scripts/backup-all.sh

# Только база данных
./scripts/backup-db.sh

# Только плагины
./scripts/backup-plugins.sh
```

### Веб-интерфейс для скачивания

1. **Откройте:** `https://ru-pills.com/backup-download.php`
2. **Введите пароль** (настройте в файле `backup-download.php`)
3. **Скачайте нужные файлы**

⚠️ **ВАЖНО:** Измените пароль в `html/backup-download.php`:
```php
$BACKUP_PASSWORD = 'your_secure_password_here';
```

## 🔄 Восстановление

### Восстановление на новом сервере

1. **Клонируйте репозиторий:**
```bash
git clone https://github.com/your-username/rupills-infra.git
cd rupills-infra
```

2. **Настройте .env:**
```bash
cp .env.example .env
nano .env
```

3. **Скачайте бэкапы:**
   - Зайдите на `https://ru-pills.com/backup-download.php`
   - Скачайте все файлы с суффиксом `-latest`

4. **Поместите бэкапы в директорию:**
```bash
mkdir -p backups
# Скопируйте скачанные файлы в backups/
```

5. **Запустите Docker:**
```bash
docker-compose up -d db redis
# Подождите 10 секунд пока БД запустится
```

6. **Восстановите всё:**
```bash
./scripts/restore.sh all
```

7. **Запустите WordPress:**
```bash
docker-compose exec web wp core download --locale=ru_RU --force
docker-compose up -d web
```

8. **Проверьте права доступа:**
```bash
docker-compose exec web chown -R lsadm:lsadm /var/www/html
```

### Частичное восстановление

```bash
# Только база данных
./scripts/restore.sh db

# Только плагины
./scripts/restore.sh plugins

# Только uploads
./scripts/restore.sh uploads

# Только тема
./scripts/restore.sh theme
```

## 🐳 Docker команды

### Основные команды

```bash
# Запуск всех сервисов
docker-compose up -d

# Остановка всех сервисов
docker-compose down

# Просмотр логов
docker-compose logs -f web

# Перезапуск сервиса
docker-compose restart web

# Выполнение команд в контейнере
docker-compose exec web wp plugin list
docker-compose exec db mysql -u wordpress -p wordpress
```

### Полезные команды WP-CLI

```bash
# Список плагинов
docker-compose exec web wp plugin list

# Обновление WordPress core
docker-compose exec web wp core update

# Очистка кэша
docker-compose exec web wp cache flush

# Оптимизация БД
docker-compose exec web wp db optimize
```

## 📤 GitHub

### Что хранится в Git

✅ **Хранится:**
- Docker конфигурация (docker-compose.yml, Dockerfile)
- Скрипты бэкапа и восстановления
- Кастомная тема `flatsome-child/`
- Конфигурационные файлы
- README и документация

❌ **НЕ хранится:**
- WordPress core (скачивается автоматически)
- Плагины (восстанавливаются из бэкапов)
- Uploads (восстанавливаются из бэкапов)
- wp-config.php (восстанавливается из бэкапа)
- База данных (восстанавливается из бэкапа)

### Первый коммит

```bash
cd /var/www
git init
git add .
git commit -m "Initial commit: Docker infrastructure for ru-pills.com"
git branch -M main
git remote add origin https://github.com/your-username/rupills-infra.git
git push -u origin main
```

### Обновление репозитория

```bash
git add .
git commit -m "Update: описание изменений"
git push
```

## 🔒 Безопасность

1. **Никогда не коммитьте:**
   - `.env` файл
   - `wp-config.php`
   - Плагины (особенно крякнутые)
   - Пароли и секретные ключи

2. **Защитите веб-интерфейс бэкапов:**
   - Измените пароль в `backup-download.php`
   - Рассмотрите добавление IP-ограничений через .htaccess

3. **Шифрование wp-config.php:**
   - Бэкапы wp-config.php автоматически шифруются
   - Пароль для расшифровки указан в логе бэкапа

## 📞 Поддержка

При проблемах проверьте:
1. Логи Docker: `docker-compose logs`
2. Логи бэкапов: `cat backups/backup-*.log`
3. Права доступа: `ls -la html/`

## 📝 Лицензия

Инфраструктура для внутреннего использования.
